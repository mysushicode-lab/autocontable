...
export const metadata = { title: 'Intégration Sage' };
  ...
  return (
    <div style={{ maxWidth: 900, margin: '0 auto', padding: '32px 24px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 8 }}>
        ...
        <img src="/logos/sage.png" alt="Sage" style={{ height: 24, width: 'auto', objectFit: 'contain' }} />
      </div>
      <p style={{ fontSize: 16, color: 'var(--d-muted)', marginBottom: 32, marginTop: 8 }}>
        Synchronisez automatiquement les écritures FactPilot vers Sage 50, Sage 100 ou Sage 1000.
      </p>

      <DocsCard id="prerequis" title="Prérequis">
        <p style={{ margin: '0 0 12px', color: 'var(--d-text)', fontSize: 14, lineHeight: 1.7 }}>
          Avant de configurer l'intégration, vérifiez que votre environnement Sage remplit les conditions suivantes :
        </p>
        <ul style={{ margin: 0, paddingLeft: 20, color: 'var(--d-muted)', fontSize: 14, lineHeight: 2 }}>
          <li><strong style={{ color: 'var(--d-text)' }}>Version :</strong> Sage 50, Sage 100 ou Sage 1000 — version 2022 ou supérieure</li>
          <li><strong style={{ color: 'var(--d-text)' }}>Accès :</strong> Droits administrateur sur le dossier Sage cible</li>
          <li><strong style={{ color: 'var(--d-text)' }}>Module :</strong> Module import/export activé dans la licence Sage</li>
          <li><strong style={{ color: 'var(--d-text)' }}>Répertoire :</strong> Un répertoire réseau accessible en lecture/écriture par FactPilot et Sage</li>
        </ul>
        <DocsNote>
          Si vous utilisez Sage en mode hébergé (cloud Sage), contactez votre revendeur Sage pour vérifier que l'accès aux répertoires de synchronisation est activé pour votre compte.
        </DocsNote>
      </DocsCard>
      ...
      <DocsCard id="configuration" title="Configuration dans FactPilot">
        <div style={STEP}>
          ...
          <div>
            ...
            <div style={{ fontSize: 14, color: 'var(--d-muted)', lineHeight: 1.6 }}>
              Depuis le menu principal, allez dans <strong>Intégrations → Sage → Configurer</strong>.
            </div>
          </div>
        </div>
        <div style={STEP}>
          ...
          <div>
            ...
            <div style={{ fontSize: 14, color: 'var(--d-muted)', lineHeight: 1.6 }}>
              Indiquez le chemin du répertoire de synchronisation partagé entre FactPilot et Sage (ex : <code style={{ background: 'var(--d-code-bg)', padding: '1px 5px', borderRadius: 4, fontSize: 12 }}>\\serveur\sage\import</code>).
            </div>
          </div>
        </div>
        <div style={STEP}>
          ...
          <div>
            ...
            <div style={{ fontSize: 14, color: 'var(--d-muted)', lineHeight: 1.6 }}>
              Sélectionnez le format <strong>Grand Livre</strong> pour les écritures comptables générées par FactPilot.
            </div>
          </div>
        </div>
        <div style={{ ...STEP, marginBottom: 8 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 14, color: 'var(--d-muted)', lineHeight: 1.6 }}>
              Cliquez sur <strong>Enregistrer et activer</strong>. Les écritures validées seront désormais exportées automatiquement dans le répertoire Sage.
            </div>
          </div>
        </div>
        <DocsNote>
          La synchronisation s'effectue à chaque validation d'écriture dans FactPilot. Vous pouvez également déclencher un export manuel depuis <strong>Dossier → Exporter → Sage</strong>.
        </DocsNote>
      </DocsCard>

      <DocsCard id="format-export" title="Format d'export">
        <p style={{ margin: '0 0 14px', color: 'var(--d-text)', fontSize: 14, lineHeight: 1.7 }}>
          FactPilot génère des fichiers texte (<code style={{ background: 'var(--d-code-bg)', padding: '1px 5px', borderRadius: 4 }}>.txt</code>) au format d'import Sage Grand Livre. Chaque ligne correspond à une ligne d'écriture comptable.
        </p>
